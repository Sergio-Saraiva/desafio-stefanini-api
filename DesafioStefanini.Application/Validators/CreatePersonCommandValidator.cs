...
namespace DesafioStefanini.Application.Validators
{
    public class CreatePersonCommandValidator : AbstractValidator<CreatePersonCommand>
    {
        public CreatePersonCommandValidator()
        {
            ...
            RuleFor(prop => prop.CityName).MinimumLength(3).WithMessage("{PropertyName} must have at leat 3 characters")
                .MaximumLength(200).WithMessage("{PropertyName} must have a maximum of 200 characters")
                .NotEmpty().WithMessage("{PropertyName} must not be empty")
                .NotNull().WithMessage("{PropertyName} must not be null");

            RuleFor(prop => prop.CityUF).MinimumLength(2).WithMessage("{PropertyName} must have at leat 2 characters")
